...
try:
    from holiday import holiday_info, Holiday  # noqa: F401
...
def main():
    ...
    with connect_sqlite(dev_db_path) as conn:
        ...
        with conn:
            ...
            if not args.dry_run:
                if key_type == "HOLIDAY":
                    mark_used_holiday(conn, row["message_id"], target_date, key_value)
                else:
                    mark_used_mmdd(conn, row["message_id"], target_date, key_value)
                print(
                    f"Marked used: message_id={row['message_id']} on {target_date} for {key_type}={key_value}"
                )

            message_id = row["message_id"]

            # For Telegram posting, integrate here if desired using TelegramPoster and 'parts'
            subj, verse_line, reading_line, reflection, prayer, verse_text = parts
            # print(f"message_id: {message_id}")
            # print(f"subject: {subj}")
            # print(f"verse_line: {verse_line}")
            # print(f"verse_text: {verse_text}")
            # print(f"show(holiday_name): {show(holiday_name)}")
            # print(f"show(holiday_emoticon): {show(holiday_emoticon)}")
