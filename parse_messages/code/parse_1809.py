#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1809)
...
def build_detection_patterns(cfg: BatchConfig) -> Dict[str, object]:
    ...
    # Build multi-header detectors (lists), tolerant to underscores and optional colon or question mark
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Directory containing .txt messages (default: 1809)",
    )
    ap.add_argument(
        "--out",
        default=BatchConfig().out_json,
        help="Output JSON file (default: parsed_1809.json)",
    )
    args = ap.parse_args()
