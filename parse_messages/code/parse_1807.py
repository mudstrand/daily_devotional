#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1807)
...
class BatchConfig:
    ...
    signature_phrase: str = r"PASTOR\s+AL"  # not used in this batch but supported
...
def default_verse_labels() -> List[str]:
    # This batch uses "VERSE FOR TODAY", with a typo variant "VERSE OR TODAY"
    return [
        r"(?:THE\s+)?VERSE\s+FOR\s+TODAY",
        r"(?:THE\s+)?VERSE\s+OR\s+TODAY",  # typo seen in July 31 sample
...
def build_detection_patterns(cfg: BatchConfig) -> Dict[str, object]:
    ...
    verse_or_line_re = re.compile(
        rf"^\s*_*\s*(?:THE\s+)?VERSE\s+OR\s*(?:{DATE_VARIANT}|TODAY)\s*[:;]?\s*_*\s*(?P<after>.*)$",
        re.IGNORECASE,
    )
    ...
    return {
        ...
        "VERSE_OR_LINE_RE": verse_or_line_re,
...
def find_positions_and_reading(
    ...
    for i, ln in enumerate(lines):
        ...
        if verse_line is None:
            if (
                line_matches_any(ln, pats["VERSE_LINE_RE_LIST"])
                or pats["VERSE_FOR_LINE_RE"].match(ln)
                or pats["VERSE_OR_LINE_RE"].match(ln)
            ):
                verse_line = i
        ...
        # Signature detection (not present here but kept)
...
def slice_sections(
    ...
    if verse_line is not None:
        ...
        if not tail:
            for m in (
                pats["VERSE_FOR_LINE_RE"].match(lines[verse_line]),
                pats["VERSE_OR_LINE_RE"].match(lines[verse_line]),
            ):
                if m:
                    t = (m.groupdict().get("after") or "").strip()
                    if t:
                        tail = t
                        break
        if tail:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Directory containing .txt messages (default: 1807)",
    )
    ap.add_argument(
        "--out",
        default=BatchConfig().out_json,
        help="Output JSON file (default: parsed_1807.json)",
    )
    args = ap.parse_args()
