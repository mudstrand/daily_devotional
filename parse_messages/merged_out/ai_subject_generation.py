...
def build_claude_messages(reflection: str) -> List[Dict[str, str]]:
    if not isinstance(reflection, str) or not reflection.strip():
        reflection = "Write a concise, encouraging devotional email subject rooted in hope, faith, and practical wisdom."
    instr = (
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                ...
                raise RuntimeError(
                    f"Claude generation failed after {retries} attempts: {e}"
                ) from e
