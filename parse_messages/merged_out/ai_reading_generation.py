...
def build_claude_messages(reflection: str, current_verse: str) -> List[Dict[str, str]]:
    ...
    context = (
        f"Existing verse (must avoid): {current_verse}\nReflection:\n{reflection}\n"
    )
    return [
...
def call_claude_for_reading(
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if not parsed:
                raise RuntimeError(
                    f"Model output not recognized as a reference: {cand!r}"
                )

            # Must differ from current verse
            if current_verse and same_reference(current_verse, cand_norm):
                raise RuntimeError(
                    f"Model suggested same verse as existing: {cand_norm!r}"
                )
            ...
            if total < 1 or total > 4:
                raise RuntimeError(
                    f"Model suggested {total} verses (must be 1–4): {cand_norm!r}"
                )
            ...
            if attempt < retries:
                ...
                raise RuntimeError(
                    f"Claude reading generation failed after {retries} attempts: {e}"
                ) from e
