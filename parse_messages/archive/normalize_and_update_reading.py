...
BOOK_CHAPTER_VERSES_RE = re.compile(
    r"^\s*(?P<book>[\dA-Za-z ]+?)\s+(?P<chapter>\d+):(?P<rest>.+?)\s*$"
)

# Match Book Chapter (whole chapter, no verses)
BOOK_WHOLE_CHAPTER_RE = re.compile(
    r"^\s*(?P<book>[\dA-Za-z ]+?)\s+(?P<chapter>\d+)\s*$"
)
        ...
        for item in value:
            ...
            if contains_chapter_reference(item):
                if is_whole_chapter(item):
                    ...
                    norm = normalize_reference_string(
                        item
                    )  # may raise ValueError for descending ranges
                    out.append(norm)
                    if norm != item:
                        changed = True
            else:
...
def update_record(
    ...
    # Normalize verse
    if (
        VERSE_FIELD in rec_copy
        and isinstance(rec_copy[VERSE_FIELD], str)
        and rec_copy[VERSE_FIELD].strip()
    ):
            ...
            if preview and not continue_on_error:
                ...
                if preview:
                    preview_entries.append(
                        (VERSE_FIELD, {"error": str(e), "before": before})
                    )
        else:
            if after != before:
                ...
                if preview:
                    preview_entries.append(
                        (VERSE_FIELD, {"before": before, "after": after})
                    )
    ...
    if READING_FIELD in rec_copy and isinstance(rec_copy[READING_FIELD], (str, list)):
        ...
        if (isinstance(before_r, str) and before_r.strip() == "") or (
            isinstance(before_r, list)
            and all((isinstance(v, str) and v.strip() == "") for v in before_r)
        ):
            pass
        else:
            try:
                ...
                if preview and not continue_on_error:
                    ...
                    if preview:
                        ...
                        preview_entries.append(
                            (READING_FIELD, {"error": str(e), "before": before_str})
                        )
            except ValueError as e:
        ...
        try:
            for idx, rec in enumerate(records, start=1):
                if not isinstance(rec, dict):
                    ...
                    continue
                upd, entries = update_record(
                    rec, args.preview, args.continue_on_error, path, idx
                )
