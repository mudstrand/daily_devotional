...
class GMailService:
    ...
    def list_message_ids(self, query: str, limit: Optional[int] = None, verbose: bool = False) -> List[str]:
        """
...
def extract_sections(
    ...
    if prayer:
        prayer = re.sub(r'^(?:\*\*|__|_)?\s*Prayer\b:?\s*', '', prayer, flags=re.IGNORECASE).strip()
    ...
    def fix(s: str) -> str:
        ...
        s = re.sub(r'\n{2,}', '\n\n', s)  # enforce exactly one blank line between paragraphs
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Parse Gmail devotional emails into verse, reflection, prayer.')
    p.add_argument(
        '--sender',
    ...
    p.add_argument('--message-id', help='Process a single message by ID (skips search).')
    p.add_argument(
        '--query',
    ...
    p.add_argument('--token', default=DEFAULT_TOKEN_FILE, help='Path to token.pickle file.')
    p.add_argument(
        '--raw-html',
        action='store_true',
...
def main():
    ...
    if args.message_id:
        ...
        ids = service.list_message_ids(query=query, limit=(args.limit or None), verbose=args.verbose)
    ...
    for idx, m in enumerate(msgs, start=1):
        try:
            ...
            if args.verbose:
                print(f'[{idx}/{total}] {status} {mid} | ok={ok_count} unid={unid_count} err={fail_count}')
            else:
                # Compact single-line progress; every 10, print a line
                if idx % 10 == 0 or idx == total:
                    print(f'[{idx}/{total}] ok={ok_count} unid={unid_count} err={fail_count}')
                else:
                    print(
            ...
            print('-' * 40)
            print(f'Date: {rec.get("date")}, Subject: {rec.get("subject")}, Message ID: {rec.get("message_id")}')
    ...
    print(f'Done. Processed {total}: OK={ok_count}, UNIDENTIFIED={unid_count}, ERRORS={fail_count} in {elapsed:.1f}s')
