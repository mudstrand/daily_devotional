...
def extract_verses_from_file(
    path: Path, include_index: bool, include_message_id: bool
) -> int:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract the 'verse' value from all records in JSON files."
    )
    parser.add_argument(
    ...
    for file_arg in args.files:
        ...
        if not path.exists():
            ...
            continue
        total += extract_verses_from_file(
            path, args.include_index, args.include_message_id
        )
