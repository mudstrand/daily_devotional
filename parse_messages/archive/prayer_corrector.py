...
def clean_claude_response(response_text: str, original_text: str) -> Optional[str]:
    ...
    for prefix in unwanted_prefixes:
        cleaned = re.sub(
            r"^\s*" + re.escape(prefix) + r"\s*", "", cleaned, flags=re.IGNORECASE
        ).strip()
    cleaned = re.sub(r"^[:~\-–—\s]+", "", cleaned)
...
def process_prayer(
    ...
    if changed:
        ...
        if preview_mode:
            ...
            print(f"\n{'-' * 60}")
            print(
                f"PRAYER update for Message ID {message_id}{cache_indicator} [{source}]"
            )
            print(f"{'-' * 60}")
            print("ORIGINAL:")
            print(f'"{original}"')
...
def main():
    parser = argparse.ArgumentParser(
        description="Correct 'prayer' field in JSON files using Claude AI"
    )
