...
def build_args():
    p = argparse.ArgumentParser(description='Load fixed devotional files into the database (fully populated).')
    p.add_argument('--dir', default=FIXED_DIR, help='Directory containing fixed files.')
    p.add_argument('--dry-run', action='store_true', help='Do not write to DB or rename files.')
    p.add_argument(
        '--sender',
