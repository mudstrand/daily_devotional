...
def process_file(path: str, mode: str, fields_to_use) -> Tuple[int, int]:
    ...
    if changed and mode in ('interactive', 'yes'):
        ...
        print(f'Updated {name} (+{file_repl} replacements in {fields_changed} field(s))')
...
def main():
    ...
    ap = argparse.ArgumentParser(description='Normalize Bible book abbreviations to full names in JSON fields.')
    ...
    group.add_argument('--yes', action='store_true', help='Apply all replacements without prompting')
    ap.add_argument(
        '--fields',
        nargs='*',
