#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1102)

Parses February 2011 “Truths ~ to live by” messages:
-   Dateline banner (mm/dd/yyyy ~~~ food for thought ~~~ title)
-   Verses (“Our Bible Verse(s) for Today:”)
-   Reflection (“Our Lesson for Today:” / “The Lesson to be Learned:” / “Our Truth for Today:” / “Thoughts to live by:”)
-   Prayer (“Prayer Suggestion:” / “Today’s Prayer:”)
...
DATE_LINE_RE = re.compile(r'^\s*\d{2}/\s?\d{2}/\d{4}.*$', re.IGNORECASE)

# Verses: Our Bible Verse(s) for Today, Verses for Today, Today's Verse(s)
VERSES_HDR_RE = re.compile(
    r"^\s*(?:Our\s+Bible\s+Verses?\s+for\s+Today|Verses?\s+for\s+Today|Today'?s\s+Verses?)\s*[:\-]?\s*$",
    re.IGNORECASE,
)

# Reflection: Our Lesson for Today, The Lesson to be Learned, Today's Lesson/Thoughts, Thoughts to live by, Our Truth for Today
THOUGHTS_HDR_RE = re.compile(
    r"^\s*(?:Our\s+Lesson\s+for\s+Today|The\s+Lesson\s+to\s+be\s+Learned|Today'?s\s+(?:Lesson|Lessons|Thoughts?)|Thoughts?\s+to\s+live\s+by|Our\s+Truth\s+for\s+Today)\s*[:\-]?\s*$",
    re.IGNORECASE,
)

# Prayer: Prayer Suggestion, Today’s Prayer, Today’s Prayer Suggestion
PRAYER_HDR_RE = re.compile(
    r"^\s*(?:Prayer\s+Suggestion|Today'?s\s+Prayer(?:\s+Suggestion)?)\s*[:\-]?\s*$",
    re.IGNORECASE,
)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Parse 1102 devotionals (Verses → Lesson/Thoughts → Prayer)')
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1102)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1102.json)',
    )
    args = ap.parse_args()
