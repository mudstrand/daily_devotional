#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0906)
...
DATE_BANNER_RE = re.compile(
    ...
    ^\s*
    (?:
        \d{2}/\d{2}/\d{2}(?:\d{2})?   # 06/12/09 or 06/12/2009
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?      # ~~~ or ~~~~
        (?:
            ...
            \s*~\s*~\s*~\s*(?:~\s*)?.*
            |
            .+                        # tolerate date + arbitrary title after tildes
        )
    )?
    \s*$
    """,
...
PRAYER_HDR_RE = re.compile(
    r'^\s*(?:Prayer\s+Suggestion|Suggested\s+Prayer|Our\s+Prayer\s+for\s+Today)\s*[:\-]?\s*$',
    re.IGNORECASE,
)
