#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1805)
...
def build_detection_patterns(cfg: BatchConfig) -> Dict[str, object]:
    ...
    # Guard against “THOUGHT FOR TODAY” stuck to a previous line (e.g., after a verse reference)
    thought_stuck_token_re = re.compile(r'\bTHOUGHT\s+FOR\s+TODAY\s*:\s*', re.IGNORECASE)
    ...
    return {
        ...
        'THOUGHT_STUCK_TOKEN_RE': thought_stuck_token_re,
    }
...
def split_stuck_thought_tail(line: str, pats: Dict[str, object]) -> tuple[str, Optional[str]]:
    """
    If the 'THOUGHT FOR TODAY:' token is accidentally stuck at the end of a verse line,
    split it and return (clean_verse_line, thought_inline_after or None).
    """
    m = pats['THOUGHT_STUCK_TOKEN_RE'].search(line)
    if not m:
        return line, None
    # left is verse portion; right is initial reflection content (if any)
...
def find_positions_and_reading(
    ...
    Returns:
      ...
      - thought_inline_seed: content that was stuck to verse line after 'THOUGHT FOR TODAY:' (or None)
    """
    verse_line = None
    thought_line = None
    prayer_line = None
    ...
    for i, ln in enumerate(lines):
        ...
        # Handle “stuck” THOUGHT token at end of a verse or verse-ref line
        if verse_line is not None and thought_line is None:
            if pats['THOUGHT_STUCK_TOKEN_RE'].search(ln):
                cleaned, seed = split_stuck_thought_tail(ln, pats)
                lines[i] = cleaned
                if seed and not thought_inline_seed:
                    thought_inline_seed = seed
                # Also mark next line as thought line if the stuck token sits exactly at this line
                thought_line = i + 0  # treat current line as the header boundary
...
def slice_sections(
    ...
    thought_inline_seed: Optional[str],
    ...
    if thought_line is not None:
        ...
        # Seed content if THOUGHT token was stuck at the end of a verse line
        if thought_inline_seed:
            chunks.append(thought_inline_seed)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help='Directory containing .txt messages (default: 1805)',
    )
    ap.add_argument(
        '--out',
        default=BatchConfig().out_json,
        help='Output JSON file (default: parsed_1805.json)',
    )
    args = ap.parse_args()
