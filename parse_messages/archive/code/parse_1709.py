...
SCRIPTURE_REF_RE = re.compile(
    r"\(([1-3]?\s?[A-Za-z][A-Za-z\.]*\s*\d+[:\.]\d+[A-Za-z0-9\-\s,]*?)\)\s*$"
)
...
def looks_like_scripture_line(line: str) -> bool:
    ...
    return bool(SCRIPTURE_REF_RE.search(ln_plain)) or bool(
        QUOTED_LINE_RE.search(ln_plain)
    )
...
def slice_with_headers(
    body: str, pats: Dict[str, object], cfg: BatchConfig
    ...
    if verse_line is not None:
        ...
        stop = min(
            [i for i in [thought_line, prayer_line, signature_line] if i is not None]
            or [len(lines)]
        )
    ...
    if thought_line is not None:
        ...
        stop = min(
            [i for i in [prayer_line, signature_line] if i is not None] or [len(lines)]
        )
        chunks = [after] if after else []
...
def parse_one(full_text: str, cfg: BatchConfig) -> Dict[str, object]:
    ...
    verse_raw, reflection_raw, prayer_raw, had_headers = slice_with_headers(
        body, pats, cfg
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--out", default=CFG.out_json, help="Output JSON (default: parsed_1709.json)"
    )
    args = ap.parse_args()
