#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0908)

Parses “thoughts to live by” notes with structure:
-       Dateline banner: mm/dd/yy or mm/dd/yyyy (tolerates minor typos like 089/10/09),
...
# - tolerate minor date typo like 089/10/09 (extra 0 before month’s 2 digits)
...
DATE_BANNER_RE = re.compile(
    ...
    ^\s*
    (?:
        \d{2,3}/\d{2}/\d{2}(?:\d{2})?   # 08/31/09 or 089/10/09 variant
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
        (?:
