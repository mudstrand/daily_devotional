#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1309)

Parses “Today’s Verses/Thoughts/Prayer Suggestion” notes, with fallbacks for non-structured items.

Extracts:
-      header fields (message_id, subject, from, to, date)
-      verse (Today’s Verses/Verse section)
-      reflection (Today’s Thoughts)
-      prayer (Today’s Prayer Suggestion)
...
# Thoughts header (allow a stray quote at end)
THOUGHTS_HDR_RE = re.compile(r"^\s*Today'?s\s+Thoughts?\s*\"?\s*[:\-]?\s*$", re.IGNORECASE)
# Prayer suggestion or short PRAYER:
...
def slice_todays_sections(body: str) -> tuple[str, str, str]:
    ...
    if idx_verses is None and idx_thoughts is None and idx_prayer is None:
        # No structured headers; treat as admin/other note
        return '', '', ''
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Parse 1309 devotionals (Today's Verses/Thoughts/Prayer Suggestion + misc.)"
    )
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1309)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1309.json)',
    )
    args = ap.parse_args()
