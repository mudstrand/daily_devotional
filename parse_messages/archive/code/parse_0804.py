#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0804)
...
# Dateline banner: tolerate mm/dd/yy, mm/dd/yyyy, and typo 11//19/08; optional "(food for thought)" segment
DATE_BANNER_RE = re.compile(
    ...
    (?:
        ...
        |
        \d{2}/\s*\d{2}/\d{2}(?:\d{2})?     # 04/08/08 or 04/ 08/2008 (tolerate stray space)
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
...
# Verse header variations (singular/plural, with/without "Our"/"Bible")
...
def main() -> None:
    ...
    ap.add_argument(
        '--skip-forwards',
        action='store_true',
        help='Skip messages that look like forwards without devotional headers',
    )
    args = ap.parse_args()
    ...
    for fp in files:
        ...
        if args.skip_forwards:
            subj = rec['subject'].lower()
            if subj.startswith('fw:') or subj.startswith('fwd:'):
                if not (rec['found_verse'] or rec['found_reflection'] or rec['found_prayer']):
                    continue
        rows.append(rec)
