#!/usr/bin/env python3
"""
Daily Devotional Message Parser (Meditation → Thought → Prayer) — batch 0708
...
DATE_BANNER_RE = re.compile(
    ...
    (?:
        ...
        |
        \d{2}/\s*\d{2}/\d{2}(?:\d{2})?     # 08/24/07 or 08/ 24/2007
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
...
# Headers (explicit, with minor flexibility for 0708 variants):
