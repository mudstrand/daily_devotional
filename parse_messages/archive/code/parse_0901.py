#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0901)
...
-      Prayer header: Prayer Suggestion / Suggested Prayer (tolerates small variants)
...
DATE_BANNER_RE = re.compile(
    ...
    ^\s*
    (?:
        \d{1,2}/\d{2}/\d{2}(?:\d{2})?      # 1/09/09 or 01/09/2009
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
        (?:
...
PRAYER_HDR_RE = re.compile(
    ...
    (?:
        ...
        Suggested\s+Prayer
        |
        Prayer\s+for\s+Today
        |
        Prayer\s+Suggestions
    )
    \s*[:\-]?\s*$
    """,
    re.IGNORECASE | re.VERBOSE,
...
def slice_sections(body: str) -> Tuple[str, str, str]:
    ...
    def block(start_idx: Optional[int], stops: List[Optional[int]]) -> str:
        ...
        # Trim trailing signature/tail lines inside block
