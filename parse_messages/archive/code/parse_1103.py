#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1103)

Parses March 2011 “Truths to live by / food for thought” notes:
-  Date line with tildes (optional)
-  Variable section headers:
  Verse: Today's Verse/Verses for Today/Our Verse for Today/Today's Scripture
  Reflection: Today's Lesson/Today's Thoughts/Thoughts to live by/Food for Thought
  Prayer: Prayer Suggestion/Today's Prayer/Today's Prayer Suggestion

Extracts:
-  header fields (message_id, subject, from, to, date)
-  verse (scripture block)
-  reflection (lesson/thought)
...
# Verse headers: Today's Verse/Verses for Today/Our Verse for Today/Today's Scripture
VERSES_HDR_RE = re.compile(
    r"^\s*(?:Today'?s\s+Verse(?:s)?|Verses?\s+for\s+Today|Our\s+Verse\s+for\s+Today|Today'?s\s+Scripture)\s*[:\-]?\s*$",
    re.IGNORECASE,
)

# Reflection headers: Today's Lesson/Thought/Thoughts, Thoughts to live by, Food for Thought
THOUGHTS_HDR_RE = re.compile(
    r"^\s*(?:Today'?s\s+(?:Lesson|Lessons|Thoughts?|Message)|Thoughts?\s+to\s+live\s+by|Food\s+for\s+Thought)\s*[:\-]?\s*$",
    re.IGNORECASE,
)

# Prayer headers: Today's Prayer/Prayer Suggestion/Today's Prayer Suggestion
PRAYER_HDR_RE = re.compile(
    r"^\s*(?:Today'?s\s+Prayer(?:\s+Suggestion)?|Prayer\s+Suggestion)\s*[:\-]?\s*$",
    re.IGNORECASE,
)

# Some items use “Question:”/“Answer:” subsections within the reflection; we keep them in reflection
# Forward/admin detection: If none of the headers is present, treat as non-structured
...
def slice_sections(body: str) -> tuple[str, str, str]:
    ...
    if idx_verses is None and idx_thoughts is None and idx_prayer is None:
        # non-structured forward/admin item
        return '', '', ''
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Parse 1103 devotionals (Verse → Lesson/Thoughts → Prayer)')
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1103)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1103.json)',
    )
    args = ap.parse_args()
