...
def build_detection_patterns(cfg: BatchConfig) -> Dict[str, re.Pattern]:
    ...
    thought_line_re_list = [
        re.compile(rf"^\s*{pat}\s*:?\s*$", re.IGNORECASE) for pat in cfg.thought_labels
    ]
    thought_join_re_list = [
        re.compile(rf"\b{pat}\s*:\s*", re.IGNORECASE) for pat in cfg.thought_labels
    ]
...
def find_positions_and_reading(
    ...
    for i, ln in enumerate(lines):
        ...
        if is_thought_header(
            ln, pats["THOUGHT_LINE_RE_LIST"], pats["THOUGHT_JOIN_RE_LIST"]
        ):
