#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0812)

Parses “thoughts to live by” notes with structure:
-      Dateline banner: mm/dd/yy or mm/dd/yyyy, optional ~~~ (food for thought) ~~~ title
      (tolerates 3–4 tildes, optional spaces/parentheses; also allows date + arbitrary title without phrase)
-      Verse header: Our/Bible Verse(s)/Verses for Today
...
# Dateline banner: mm/dd/yy or mm/dd/yyyy, optional "~~~ (food for thought) ~~~ ..." tolerant
DATE_BANNER_RE = re.compile(
    r"""
    ^\s*
    (?:
        \d{2}/\d{2}/\d{2}(?:\d{2})?          # 12/01/08 or 12/01/2008
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?             # ~~~ or ~~~~
        \s*\(?\s*food\s+for\s+thought\s*\)?  # (food for thought) optional parentheses/case
        \s*~\s*~\s*~\s*(?:~\s*)?
        .*                                   # title
        |
        \s*~\s*~\s*~\s*(?:~\s*)?.*           # tolerate date + ~~~ + arbitrary title
    )?
    \s*$
    """,
