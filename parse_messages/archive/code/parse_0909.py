#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0909)

Parses “thoughts to live by” notes with structure:
-      Dateline banner: mm/dd/yy or mm/dd/yyyy (tolerates minor punctuation like 09/15'09),
      optional ~~~ FOOD FOR THOUGHT ~~~ title (3–4 tildes, optional spaces), or date + title without phrase
...
# - tolerate minor punctuation like 09/15'09 (apostrophe between day and year)
# - optional FOOD FOR THOUGHT segment with 3–4 tildes and optional spaces, or
# - date + arbitrary title after tildes
DATE_BANNER_RE = re.compile(
    r"""
    ^\s*
    (?:
        \d{2}/\d{2}/\d{2}(?:\d{2})?      # 09/12/09 or 09/12/2009
        |
        \d{2}/\d{2}['’]\d{2}            # 09/15'09 variant
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
        (?:
