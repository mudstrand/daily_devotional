#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batches 1004–1012)

Parses “thoughts to live by” notes with structure:
-      Dateline banner: mm/dd/yyyy, mm?dd/yyyy, or mm/dd/yy, optionally with a weekday in parentheses,
      followed by optional ~~~ FOOD FOR THOUGHT ~~~ title
...
# Dateline banner: allow mm/dd/yyyy, mm?dd/yyyy, and mm/dd/yy; optional weekday in parentheses; optional FOOD FOR THOUGHT suffix.
DATE_BANNER_RE = re.compile(
    r"""
    ^\s*
    (?:
        \d{2}/\d{2}/\d{4}        # 04/22/2010
        |
        \d{2}[^0-9]\d{2}/\d{4}  # 11?02/2010
        |
        \d{2}/\d{2}/\d{2}       # 10/17/24 (rare malformed)
    )
    (?:\s*\([A-Za-z]+\))?       # optional weekday, e.g., (Thursday)
    (?:\s*~\s*~\s*~\s*
...
# Verse header variants (singular and plural, case-insensitive)
...
# Reflection header variants (include “A Lesson to be learned” and case-insensitive)
