...
-    Reflection header: Our/The Lesson for Today (also: A Lesson to be learned)
-    Prayer header: Prayer Suggestion (also: Suggested Prayer)
...
# Dateline: handle 11/28/2010, 11?02/2010, and optional FOOD FOR THOUGHT block
DATE_LINE_RE = re.compile(
    r"""
    ^\s*
    (?:\d{2}[^0-9]\d{2}/\d{4}|\d{2}/\d{2}/\d{4})
...
# Signature marker and tail lines that often follow
...
def slice_sections(body: str) -> Tuple[str, str, str]:
    ...
    def block(start_idx: Optional[int], stops: List[Optional[int]]) -> str:
        if start_idx is None:
            return ''
        # Candidates to stop at: next header, signature, tail verse
        after = [x for x in stops if x is not None and x > start_idx]
        stop = min(after) if after else len(lines)
        # Trim trailing tail lines within the captured block
        content = lines[start_idx + 1 : stop]
        # Remove any trailing signature/tail in the content itself
