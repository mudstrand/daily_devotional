#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0802)

Parses “thoughts to live by” notes with structure:
-                Dateline banner: mm/dd/yy or mm/dd/yyyy (tolerates stray spaces and 11//19/08), optional ~~~ (food for thought) ~~~ title
                Also tolerates arbitrary text before the date (e.g., "Fri. is here!!!~~~" or "Hi!!!~~~") by not requiring the banner to match strictly.
...
# Flexible date banner: allows "02/28/08", "02/ 28/2008", and "11//19/08"
DATE_BANNER_RE = re.compile(
    ...
    (?:
        ...
        |
        \d{2}/\s*\d{2}/\d{2}(?:\d{2})?     # 02/28/08 or 02/ 28/2008
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
...
def slice_sections(body: str) -> Tuple[str, str, str]:
    ...
    start = 0
    # Only skip the first line if it clearly matches a date banner;
    # otherwise keep it (some lines begin with text before the banner).
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Parse devotionals (Verses → Lesson/Thought → Prayer)')
    ap.add_argument(
        '--input-dir',
