#!/usr/bin/env python3
"""
Daily Devotional Message Parser (Meditation → Thought → Prayer)
...
-     Reflection section header: Our Thought for Today:
...
@dataclass
class BatchConfig:
    # Set these per batch (e.g., "0709", "0710", ...). Defaults here:
    input_dir: str = '0709'
    out_json: str = 'parsed_0709.json'
...
DATE_BANNER_RE = re.compile(
    ...
    (?:
        ...
        |
        \d{2}/\s*\d{2}/\d{2}(?:\d{2})?     # 09/12/07 or 09/ 12/2007
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
...
# Headers (explicit, per your specification):

# Verse section begins with: Our Meditation for <x>:
# Accepts: Today / Mon. / Tue. / the Day / etc. Any text until colon.
MEDITATION_HDR_RE = re.compile(
    r"""
    ^\s*
    Our\s+Meditation\s+for\b
    .*?                             # any descriptor until the colon
    \s*:\s*$
    """,
    re.IGNORECASE | re.VERBOSE,
)

# Reflection section begins with: Our Thought for Today:
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Parse devotionals (Meditation → Thought → Prayer)')
    ap.add_argument(
        '--input-dir',
