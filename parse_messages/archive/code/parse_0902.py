#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0902)

Parses “thoughts to live by” notes with structure:
-      Dateline banner: optional weekday prefix (Mon./Tue./Wed./Thur./Fri./Sat./Sun.), then mm/dd/yy or mm/dd/yyyy,
      optional ~~~ FOOD FOR THOUGHT ~~~ title (3–4 tildes, optional spacing), or date + title without phrase
-      Verse header: Our/Bible Verse(s) for Today
-      Reflection header: Our/The Lesson for Today, or A Lesson to be learned
-      Prayer header: Prayer Suggestion or Suggested Prayer (tolerates small variants)
...
# Weekday prefix patterns (optional), followed by date, optional FOOD FOR THOUGHT segment or arbitrary title
DATE_BANNER_RE = re.compile(
    r"""
    ^\s*
    (?:(?:Mon|Tue|Wed|Thur|Thu|Fri|Sat|Sun)\.\s*)?   # optional weekday prefix like "Wed. "
    (?:
        \d{2}/\d{2}/\d{2}(?:\d{2})?                # 02/26/09 or 02/26/2009
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
        (?:
...
# Prayer header variants (tolerate slight phrasing)
PRAYER_HDR_RE = re.compile(
    r'^\s*(?:Prayer\s+Suggestion|Suggested\s+Prayer|A\s+Prayer\s+Suggestion\.?)\s*[:\-]?\s*$',
    re.IGNORECASE,
)
