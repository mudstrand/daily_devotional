#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1402)

Handles “Today’s Verses/Thoughts/Prayer Suggestion” format.

Extracts:
-    header fields (message_id, subject, from, to, date)
-    verse (from “Today’s Verses/Verse” section)
-    reflection (from “Today’s Thoughts”)
-    prayer (from “Today’s Prayer Suggestion”)
...
class BatchConfig:
    input_dir: str = '1402'
    out_json: str = 'parsed_1402.json'
    header_body_sep: str = '=' * 67
    signature_name: str = r'Pastor\s+Alvin\s+and\s+Marcie\s+Sather'
...
# Allow variations: Today's Verse(s) with punctuation/spaces
VERSES_HDR_RE = re.compile(r"^\s*Today'?s\s+Verse(?:s)?\s*[:\-]\s*", re.IGNORECASE)
THOUGHTS_HDR_RE = re.compile(r"^\s*Today'?s\s+Thoughts?\s*[:\-]\s*$", re.IGNORECASE)
PRAYER_HDR_RE = re.compile(r"^\s*Today'?s\s+Prayer\s+Suggestion\s*[:\-]\s*$", re.IGNORECASE)
# Some lines mark “THOUGHT:” and “PRAYER:” short forms in this batch too
ALT_THOUGHT_SHORT_RE = re.compile(r'^\s*THOUGHT\s*[:\-]\s*$', re.IGNORECASE)
...
def slice_todays_sections(body: str) -> tuple[str, str, str]:
    """
    Returns (verse, reflection, prayer) or empty strings if no “Today’s …” headers found.
    """
    ...
    for i, ln in enumerate(lines):
        if idx_verses is None and VERSES_HDR_RE.match(ln):
            idx_verses = i
        if idx_thoughts is None and (THOUGHTS_HDR_RE.match(ln) or ALT_THOUGHT_SHORT_RE.match(ln)):
            idx_thoughts = i
        if idx_prayer is None and (PRAYER_HDR_RE.match(ln) or ALT_PRAYER_SHORT_RE.match(ln)):
            idx_prayer = i
        if idx_signature is None and SIGNATURE_RE.match(ln):
            idx_signature = i

    # If no “Today’s …” headers, return empty => will be handled elsewhere or as admin note
    if idx_verses is None and idx_thoughts is None and idx_prayer is None:
        return '', '', ''

    # Slice blocks; stop before next header or signature
    def block(start_idx: Optional[int], stop_candidates: List[Optional[int]]) -> str:
        if start_idx is None:
            return ''
        # any next header after start
        stops = [x for x in stop_candidates if x is not None and x > start_idx]
        stop = min(stops) if stops else (idx_signature if idx_signature and idx_signature > start_idx else len(lines))
        # content starts next line after the header
        content_lines = lines[start_idx + 1 : stop]
    ...
    # Remove any trailing postscript blocks (signature scripture quote lines), if captured
    # Not strictly necessary as we stop before signature, but just in case
...
def parse_one(full_text: str) -> Dict[str, object]:
    ...
    # If nothing found and body seems like an admin note (no structured headers), keep fields empty
    ...
    rec: Dict[str, object] = {
        ...
        'found_verse': bool(verse),
        'found_reflection': bool(reflection),
        'found_prayer': bool(prayer),
        'found_reading': False,
    }
    return rec
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Parse 1402 devotionals (Today's Verses/Thoughts/Prayer Suggestion)")
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1402)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1402.json)',
    )
    args = ap.parse_args()
