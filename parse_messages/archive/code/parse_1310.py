#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1310)

Parses “Today’s Verses/Thoughts/Prayer Suggestion” notes, with fallbacks for admin/forward items.
...
def slice_todays_sections(body: str) -> tuple[str, str, str]:
    ...
    # No “Today’s …” headers => likely admin/forward; return empties
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Parse 1310 devotionals (Today's Verses/Thoughts/Prayer Suggestion + admin)"
    )
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1310)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1310.json)',
    )
    args = ap.parse_args()
