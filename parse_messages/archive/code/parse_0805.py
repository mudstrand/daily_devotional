#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0805)
...
DATE_BANNER_RE = re.compile(
    ...
    (?:
        ...
        |
        \d{2}/\d{2}/\d{2}(?:\d{2})?        # 05/02/08 or 05/02/2008
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
...
VERSES_HDR_RE = re.compile(
    ...
    (?:
        ...
        |
        Our\s+Bible\s+Verses?\s+For\s+Today   # tolerate capitalized For
    )
    \s*[:\-]?\s*$
    """,
    re.IGNORECASE | re.VERBOSE,
