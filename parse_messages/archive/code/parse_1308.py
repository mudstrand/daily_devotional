...
# Verses/Verse (also tolerate “Verse's” typo)
...
def slice_todays_sections(body: str) -> tuple[str, str, str]:
    ...
    if idx_verses is None and idx_thoughts is None and idx_prayer is None:
        # Not a structured note (none in your 1308 set)
        return '', '', ''
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Parse 1308 devotionals (Today's Verse[s]/Thoughts/Prayer Suggestion)")
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1308)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1308.json)',
    )
    args = ap.parse_args()
