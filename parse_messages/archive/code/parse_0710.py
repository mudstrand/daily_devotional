#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0710)

Parses “thoughts to live by” notes with structure:
-                     Optional dateline banner: mm/dd/yy or mm/dd/yyyy (tolerates stray spaces and 11//19/08), optional ~~~ (food for thought) ~~~ title
...
THOUGHTS_HDR_RE = re.compile(
    ...
    (?:
        ...
        |
        Our\s+Meditation\s+for\s+Today
        |
        Our\s+Meditation\s+for\s+the\s+Day
        |
        Meditation\s+for\s+Today
    )
    \s*[:\-]?\s*$
    """,
    re.IGNORECASE | re.VERBOSE,
