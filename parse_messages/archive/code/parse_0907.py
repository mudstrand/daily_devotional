#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0907)

Parses “thoughts to live by” notes with structure:
-      Dateline banner: mm/dd/yy or mm/dd/yyyy; also tolerates mm/ddyy (e.g., 07/1209) and optional ~~~ FOOD FOR THOUGHT ~~~ title
      (3–4 tildes, optional spaces), or date + title without phrase
-      Verse header: Our/Bible Verse(s) for Today
-      Reflection header: Our/The Lesson for Today, A Lesson to be learned, or Lesson for Today
-      Prayer header: Prayer Suggestion / Suggested Prayer (case/wording tolerant)
...
# - also allow mm/ddyy variant (e.g., 07/1209) where the slash before year is missing
# - optional FOOD FOR THOUGHT segment with 3–4 tildes and optional spaces, or date + arbitrary title
DATE_BANNER_RE = re.compile(
    ...
    (?:
        \d{2}/\d{2}/\d{2}(?:\d{2})?
        |
        \d{2}/\d{4}                # 07/1209 malformed variant
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
        (?:
...
# Reflection header variants (include short "Lesson for Today")
...
PRAYER_HDR_RE = re.compile(
    r'^\s*(?:Prayer\s+Suggestion|Suggested\s+Prayer|Our\s+Prayer\s+Suggestion|Prayer\s+suggestion)\s*[:\-]?\s*$',
    re.IGNORECASE,
)

# Signature and tail verse markers
SIGNATURE_RE = re.compile(rf'^\s*{CFG.signature_name}\s*$', re.IGNORECASE)
TAIL_VERSE_RE = re.compile(r'^\s*The\s+Lord\s+said,?\s*".*?"\s*~?\s*-?\s*John[: ]\s*\d+:\d+\s*$', re.IGNORECASE)
...
def slice_sections(body: str) -> Tuple[str, str, str]:
    ...
    for i in range(start, len(lines)):
        ...
        if idx_prayer is None and PRAYER_HDR_RE.match(ln):
            idx_prayer = i
            continue
        if idx_signature is None and (SIGNATURE_RE.match(ln) or TAIL_VERSE_RE.match(ln)):
            idx_signature = i
            continue
    ...
    def block(start_idx: Optional[int], stops: List[Optional[int]]) -> str:
        ...
        # Trim trailing signature/tail lines in the block
        while content and (SIGNATURE_RE.match(content[-1]) or TAIL_VERSE_RE.match(content[-1])):
            content.pop()
        return '\n'.join(content).strip()
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Parse devotionals (Verses → Lesson → Prayer)')
    ap.add_argument(
        '--input-dir',
