#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1306)

Parses “Today’s Verse/Verses → Thoughts/Food For Thought/Lesson → Prayer” notes.

Extracts:
-      header fields (message_id, subject, from, to, date)
-      verse (Today’s Verse/Verses section, including any reference lines)
-      reflection (Today’s Thoughts/Food For Thought/Lesson)
-      prayer (Today’s Prayer Suggestion or Today’s Suggested Prayer)
...
class BatchConfig:
    input_dir: str = '1306'
    out_json: str = 'parsed_1306.json'
    header_body_sep: str = '=' * 67
    # Accept “Pastor Alvin and Marcie Sather” and “Pastor Al & Marcie Sather”
...
# Verses/Verse (CASE: Verse or Verses, allow optional quotes)
VERSES_HDR_RE = re.compile(r"^\s*Today'?s\s+Verse(?:s)?\s*[:\-]\s*", re.IGNORECASE)

# Thoughts/Food for Thought/Lesson, allow typos (Toda's), optional quotes
THOUGHTS_HDR_RE = re.compile(
    r"^\s*(?:Today'?s\s+(?:Thoughts?|Food\s+For\s+Thought|Lesson)|Toda'?s\s+Thoughts?)\s*\"?\s*[:\-]?\s*$",
    re.IGNORECASE,
)

# Prayer Suggestion / Suggested Prayer
PRAYER_HDR_RE = re.compile(
    r"^\s*Today'?s\s+(?:Prayer\s+Suggestion|Suggested\s+Prayer)\s*[:\-]\s*$",
    re.IGNORECASE,
)

# Short forms (not seen in 1306, kept for robustness)
...
# Lines to ignore at the tail (common devos/admin notes)
TAIL_NOTE_RE = re.compile(r'^\s*(I will be|I will be leaving|Until Tue|Ps\s*\()', re.IGNORECASE)
...
def slice_todays_sections(body: str) -> tuple[str, str, str]:
    ...
    # If no structure headers, return empty -> leave content in original_content only
    ...
    def block(start_idx: Optional[int], stops: List[Optional[int]]) -> str:
        if start_idx is None:
            return ''
        # Find the earliest next header/signature after start
        ...
        # Content is all lines after the header up to stop
        chunk = '\n'.join(lines[start_idx + 1 : stop]).strip()
        # Remove obvious trailing administrative notes within the chunk
        chunk_lines = [cl for cl in chunk.splitlines() if cl.strip() and not TAIL_NOTE_RE.match(cl)]
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Parse 1306 devotionals (Today's Verse/Verses → Thoughts/Food For Thought/Lesson → Prayer)"
    )
    ap.add_argument(
        '--input-dir',
        default=CFG.input_dir,
        help='Directory containing .txt messages (default: 1306)',
    )
    ap.add_argument(
        '--out',
        default=CFG.out_json,
        help='Output JSON file (default: parsed_1306.json)',
    )
    args = ap.parse_args()
