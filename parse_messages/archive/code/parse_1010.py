#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batches 1010-1012)

Parses “thoughts to live by” notes with structure:
-     Dateline banner: mm/dd/yyyy or mm?dd/yyyy, optionally followed by ~~~ FOOD FOR THOUGHT ~~~ title
...
# Dateline banner: allow 10/17/24 (malformed), mm/dd/yyyy, mm?dd/yyyy, optional FOOD FOR THOUGHT suffix
DATE_BANNER_RE = re.compile(
    r"""
    ^\s*
    (?:
        \d{2}/\d{2}/\d{4}        # 10/21/2010
        |
        \d{2}[^0-9]\d{2}/\d{4}  # 11?02/2010
        |
        \d{2}/\d{2}/\d{2}       # 10/17/24 (malformed but used like a banner)
    )
...
# Prayer header variants (include “Suggested Prayer” and “Suggested Prayer:” seen in samples)
