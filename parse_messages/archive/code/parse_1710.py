#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 1710)
...
class BatchConfig:
    ...
    signature_phrase: str = r'PASTOR\s+AL'  # supported if it appears
...
def default_verse_labels() -> List[str]:
    # "VERSE FOR TODAY" and plural "VERSES FOR TODAY"
    return [
...
def default_prayer_labels() -> List[str]:
    return [
        ...
        r'PRAYER\s+FOR\s+TODAY\.',  # sometimes ends with a period
...
def build_detection_patterns(cfg: BatchConfig) -> Dict[str, object]:
    ...
    def compile_lines(patterns: List[str]) -> List[re.Pattern]:
        # Accept :, ;, . or no punctuation after the label (to catch "VERSE FOR TODAY.")
    ...
    verse_for_line_re = re.compile(
        rf'^\s*_*\s*(?:THE\s+)?VERSE[S]?\s+FOR\s*(?:{DATE_VARIANT}|TODAY)\s*[:;.]?\s*_*\s*(?P<after>.*)$',
        re.IGNORECASE,
    )

    # Signature (not present in your snippets, but kept)
...
def find_positions_and_reading(
    ...
    for i, ln in enumerate(lines):
        ...
        # Signature detection (supported if present)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help='Directory containing .txt messages (default: 1710)',
    )
    ap.add_argument(
        '--out',
        default=BatchConfig().out_json,
        help='Output JSON file (default: parsed_1710.json)',
    )
    args = ap.parse_args()
