#!/usr/bin/env python3
"""
Daily Devotional Message Parser (batch 0801)
...
-                 Dateline banner: mm/dd/yy or mm/dd/yyyy (tolerates stray spaces and 11//19/08), optional ~~~ (food for thought) ~~~ title
                 Banner is optional; parser does not require it to extract sections.
...
# Flexible date banner: allows "01/17/08", "01/ 17/2008", and "11//19/08"
DATE_BANNER_RE = re.compile(
    ...
    (?:
        ...
        |
        \d{2}/\s*\d{2}/\d{2}(?:\d{2})?     # 01/17/08 or 01/ 17/2008
    )
    (?:
        \s*~\s*~\s*~\s*(?:~\s*)?
