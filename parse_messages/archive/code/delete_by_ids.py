...
def delete_by_message_id(obj: Any, target_id: str) -> Tuple[Any, int]:
    ...
    if isinstance(obj, list):
        before = len(obj)
        obj = [rec for rec in obj if not (isinstance(rec, dict) and rec.get('message_id') == target_id)]
    ...
    if isinstance(obj, dict):
        if isinstance(obj.get('content'), list):
            ...
            obj['content'] = [
                rec for rec in obj['content'] if not (isinstance(rec, dict) and rec.get('message_id') == target_id)
            ]
...
def main() -> None:
    ...
    if not ids:
        print('No message_ids to delete (file empty or only comments).', file=sys.stderr)
        sys.exit(0)
