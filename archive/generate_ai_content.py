...
def main():
    parser = argparse.ArgumentParser(
        description="Generate AI content for devotionals using database.py (SQLite)."
    )
    parser.add_argument(
        "--table", default="devotionals", help="Table name (default: devotionals)"
    )
    parser.add_argument(
        "--column", required=True, help="Target column to populate (e.g., prayer)"
    )
    parser.add_argument(
        "--reflections-column",
        default="reflection",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview only; no changes written"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.4, help="Creativity (default: 0.4)"
    )
    parser.add_argument(
        "--max-tokens", type=int, default=200, help="Max tokens (default: 200)"
    )
    parser.add_argument(
        "--avoid-memory",
        type=int,
    ...
    with database.get_conn(database.DB_PATH) as conn:
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            if not reflection:
                print(
                    f"Skipping ID={row_id} because {args.reflections_column} is empty."
                )
                processed += 1
                continue
            ...
            avoid_list = (
                list(recent_openers)
                if recent_openers.maxlen and len(recent_openers) > 0
                else None
            )
            ...
            if args.non_interactive:
                if args.dry_run:
                    print_row_preview(
                        idx, row_id, args.column, current_value, ai_value, reflection
                    )
                    processed += 1
                    continue
                update_target_column(
                    conn, args.table, args.id_column, row_id, args.column, ai_value
                )
                updated += 1
                processed += 1
                continue
