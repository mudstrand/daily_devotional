...
def make_verse_id(book_id: int, chapter: int, verse: int) -> int:
    return int(
        f"{book_id:02d}{chapter:03d}{verse:03d}"
        if book_id < 100
        else f"{book_id:03d}{chapter:03d}{verse:03d}"
    )
...
def fetch_single_verse(
    book_id: int, chapter: int, verse: int, translation: str
) -> dict:
...
def main():
    ...
    print(
        f"[INFO] Using API base={API_BASE}, translation={args.translation}, preview={args.preview}"
    )

    for i, ref in enumerate(refs, start=1):
        try:
            store_reference(
                conn, ref, args.translation, args.sleep, books_lut, preview=args.preview
            )
        except Exception as e:
