...
pg_url = os.getenv('BIBLE_VERSE_DATABASE_URL')  # e.g., postgresql+psycopg://bible:bible@127.0.0.1:5432/bible
...
with src.connect() as sconn, dst.begin() as dconn:
    rows = sconn.execute(text('SELECT book, chapter, verse, translation, text FROM verses')).mappings().all()
    for r in rows:
        dconn.execute(
            text("""
